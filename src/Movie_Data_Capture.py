...
def movie_lists(source_folder, regexstr: str) -> typing.List[str]:
    ...
    for full_name in source.glob(r"**/*"):
        ...
        movie_size = (
            0 if is_sym else full_name.stat().st_size
...
def main():
    ...
    if rerun_delay > 0 and conf.stop_counter() > 0:
        while True:
            try:
                logfile = _main(args)
                (total_movies, movies_dealed, movies_completed) = result_tuple = tuple(
                    analysis_logfile(logfile)
                )
                if all(isinstance(v, int) for v in result_tuple):
                    last = total_movies - movies_dealed
                    total_time = timedelta(seconds=time.time() - app_start)
                    print(
                        f"All movies:{total_movies}  processed:{movies_dealed}  successes:{movies_completed}  remain:{last}"
                        + "  Elapsed time {}".format(
                            period(total_time, "{d} day {h}:{m:02}:{s:02}")
                            if total_time.days == 1
                            else period(total_time, "{d} days {h}:{m:02}:{s:02}")
                            if total_time.days > 1
                            else period(total_time, "{h}:{m:02}:{s:02}")
                        )
                    )
                    if last == 0:
                        break
                    next_runtime = datetime.now() + timedelta(seconds=rerun_delay)
                    print(
                        f'Next run time: {next_runtime.strftime("%H:%M:%S")}, rerun_delay={rerun_delay}, press Ctrl+C stop run.'
                    )
