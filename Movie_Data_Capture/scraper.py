        ...
        for translate_value in translate_values:
            ...
            if translate_value == "title":
                title_dict = json.loads(
                    (
                        Path.home() / ".local" / "share" / "mdc" / "c_number.json"
    ...
    if open_cc:
        ...
        def convert_list(mapping_data, language, vars):
            ...
            for i in vars:
                if (
                    len(
                        mapping_data.xpath(
                            "a[contains(@keyword, $name)]/@" + language, name=f",{i},"
                        )
                    )
                    != 0
                ):
        ...
        def convert(mapping_data, language, vars):
            if (
                len(
                    mapping_data.xpath(
                        "a[contains(@keyword, $name)]/@" + language, name=vars
                    )
                )
                != 0
            ):
                return mapping_data.xpath(
                    "a[contains(@keyword, $name)]/@" + language, name=vars
                )[0]
            else:
            ...
            if cc == "actor":
                try:
                    ...
                    json_data["actor_list"] = [
                        open_cc.convert(aa) for aa in json_data["actor_list"]
                    ]
...
replace_table = {
    ...
    for k, v in {
        ...
        "?": "？",  # U+FF1F FULLWIDTH QUESTION MARK @ Halfwidth and Fullwidth Forms
        '"': "＂",  # U+FF02 FULLWIDTH QUOTATION MARK @ Halfwidth and Fullwidth Forms
        "<": "ᐸ",  # U+1448 CANADIAN SYLLABICS WEST-CREE P @ Unified Canadian Aboriginal Syllabics
        ">": "ᐳ",  # U+1433 CANADIAN SYLLABICS WEST-CREE L @ Unified Canadian Aboriginal Syllabics
        "|": "ǀ",  # U+01C0 LATIN LETTER DENTAL CLICK @ Latin-1 Supplement
        "&lsquo;": "‘",  # U+2018 LEFT SINGLE QUOTATION MARK @ General Punctuation
        "&rsquo;": "’",  # U+2019 RIGHT SINGLE QUOTATION MARK @ General Punctuation
        "&hellip;": "…",  # U+2026 HORIZONTAL ELLIPSIS @ General Punctuation
        "&amp;": "＆",  # U+FF06 FULLWIDTH AMPERSAND @ Halfwidth and Fullwidth Forms
    }.items()
}
...
def special_characters_replacement(text) -> str:
    if not isinstance(text, str):
        return text
    return pattern.sub(lambda m: replace_table[re.escape(m.group(0))], text)
